...
// Detect if running as native Android app (Capacitor)
const isNativeApp = () => !!(window.Capacitor && window.Capacitor.isNativePlatform && window.Capacitor.isNativePlatform());

// Native app: auto sign-in anonymously (data is shared via HOUSEHOLD_UID anyway)
// Browser/PWA: use Google Sign-In
if (isNativeApp()) {
...
document.getElementById('google-signin-btn').addEventListener('click', async () => {
  try {
    if (isNativeApp()) {
      // Already signed in anonymously — just proceed
...
document.getElementById('sign-out-btn').addEventListener('click', async () => {
  ...
  if (!isNativeApp()) await auth.signOut();
});
...
function onLogin(user) {
  ...
  // Native app shows "Arivu" as name; Google auth users show their real name
  document.getElementById('header-name').textContent =
    user.displayName?.split(' ')[0] || (isNativeApp() ? 'Arivu' : 'User');
